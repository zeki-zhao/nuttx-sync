  ...
  uint8_t color)
{
  ...
  b16_t dx1dy;
  b16_t dx2dy;
  uint8_t mpixel = ((uint8_t)(color) << 6 | (uint8_t)(color) << 4 | (uint8_t)(color) << 2 | (color));
  uint8_t mask;
  int lnlen;
  stride = bwnd->stride;
  x1 = trap->top.x1;
  ...
  if (nrows > 1)
    {
      dx1dy = (((trap->bot.x1 - x1))/(nrows - 1));
      dx2dy = (((trap->bot.x2 - x2))/(nrows - 1));
    }
  else
    {
      x1 = (x1 + trap->bot.x1) >> 1;
  ...
  line = ( uint8_t *)bwnd->fbmem + y1 * stride ;
  while (nrows--)
    {
      int ix1;
      int ix2;
      if (x1 > x2)
        {
          b16_t tmp;
          do { tmp = x1; x1 = x2; x2 = tmp; } while (0);
          do { tmp = dx1dy; dx1dy = dx2dy; dx2dy = tmp; } while (0);
        }
      ix1 = ((x1) >> 16);
      ix2 = ((x2) >> 16);
      if (x1 <= x2 && ix2 >= bounds->pt1.x && ix1 <= bounds->pt2.x)
        {
          ix1 = ((ix1) < (bounds->pt1.x) ? (bounds->pt1.x) : (ix1));
          ix2 = ((ix2) > (bounds->pt2.x) ? (bounds->pt2.x) : (ix2));
          width = ix2 - ix1 + 1;
          mask = (uint8_t)(0xff >> (8 - ((ix1) & 3)));
          dest = line;
          lnlen = width;
          if (lnlen > 1 && mask)
            {
          ...
          if (lnlen > 0)
            {
              { uint8_t *_ptr = ( uint8_t*)(dest); int _nby = ((lnlen) >> 2); while (_nby--) { *_ptr++ = ((uint8_t)color); } };
            }
        }
      line += stride;
      x1 += dx1dy;

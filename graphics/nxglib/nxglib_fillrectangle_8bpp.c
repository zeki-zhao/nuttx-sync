  ...
  uint8_t color)
{
  ...
  line = pinfo->fbmem + rect->pt1.y * stride + (rect->pt1.x);
  while (rows-- > 0)
    {
      { uint8_t *_ptr = ( uint8_t*)(line); nxgl_coord_t _npix = (width); while (_npix--) { *_ptr++ = ((uint8_t)color); } };
      line += stride;
    }
}

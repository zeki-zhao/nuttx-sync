      ...
      if (x1 <= x2 && ix2 >= bounds->pt1.x && ix1 <= bounds->pt2.x)
        {
          ...
          width = ix2 - ix1 + 1;
          dest = line + (((ix1) << 1) + (ix1));
          { uint8_t *_ptr = ( uint8_t*)(dest); nxgl_coord_t _npix = (width); while (_npix--) { *_ptr++ = ((uint32_t)color); *_ptr++ = ((uint32_t)color) >> 8; *_ptr++ = ((uint32_t)color) >> 16; } };
        }
      line += stride;
      x1 += dx1dy;
      x2 += dx2dy;

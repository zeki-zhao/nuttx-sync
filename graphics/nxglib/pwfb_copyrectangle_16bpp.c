  ...
  unsigned int srcstride)
{
  ...
  while (rows--)
    {
      { uint16_t *_dptr = ( uint16_t*)((uint16_t *)dline); uint16_t *_sptr = ( uint16_t*)((uint16_t *)sline); nxgl_coord_t _npix = (width); while (_npix--) { *_dptr++ = *_sptr++; } };
      dline += deststride;
      sline += srcstride;
    }
}

...
void nxglib_cursor_backup_32bpp( struct nxbe_state_s *be,
                                const struct nxgl_rect_s *bounds,
                                int planeno);
void nxglib_cursor_backup_24bpp
(
  struct nxbe_state_s *be,
  const struct nxgl_rect_s *bounds,
  int planeno)
  ...
  if (!nxgl_nullrect(&intersection))
    {
      ...
      dstride = (((dwidth) << 1) + (dwidth));
      nxgl_vectsubtract(&origin, &intersection.pt1, &be->cursor.bounds.pt1);
      fbmem = ( uint8_t *)plane->pinfo.fbmem;
      sline = ( uint8_t *)fbmem + sstride * intersection.pt1.y +
                (((intersection.pt1.x) << 1) + (intersection.pt1.x));
      dline = ( uint8_t *)be->cursor.bkgd + dstride * origin.y +
                (((origin.x) << 1) + (origin.x));

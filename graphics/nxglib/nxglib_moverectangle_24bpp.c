  ...
  struct nxgl_point_s *offset)
{
  ...
  sline = pinfo->fbmem + rect->pt1.y * stride + (((rect->pt1.x) << 1) + (rect->pt1.x));
  dline = pinfo->fbmem + offset->y * stride + (((offset->x) << 1) + (offset->x));
  if (offset->y < rect->pt1.y ||
     (offset->y < rect->pt1.y && offset->x <= rect->pt1.x))
    {
      while (rows--)
        {
          { uint8_t *_dptr = ( uint8_t*)(dline); uint8_t *_sptr = ( uint8_t*)(sline); nxgl_coord_t _npix = (width); while (_npix--) { *_dptr++ = *_sptr++; *_dptr++ = *_sptr++; *_dptr++ = *_sptr++; } };
          dline += stride;
          sline += stride;
        }
    }

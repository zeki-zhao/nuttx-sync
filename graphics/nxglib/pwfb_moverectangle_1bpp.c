...
static inline void pwfb_lowresmemcpy( uint8_t *dline,
                                     const uint8_t *sline,
  ...
  struct nxgl_point_s *offset)
{
  ...
  sline = ( uint8_t *)bwnd->fbmem + rect->pt1.y * stride +
          ((rect->pt1.x) >> 3);
  dline = ( uint8_t *)bwnd->fbmem + offset->y * stride +
          ((offset->x) >> 3);

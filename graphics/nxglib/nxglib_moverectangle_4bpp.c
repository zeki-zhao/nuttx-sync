                                     ...
                                     uint8_t tailmask)
{
  ...
  if (lnlen > 0)
    {
      { uint8_t *_dptr = ( uint8_t*)(dptr); uint8_t *_sptr = ( uint8_t*)(sptr); int _nby = ((lnlen) >> 1); while (_nby--) { *_dptr++ = *_sptr++; } };
    }
}
void nxgl_moverectangle_4bpp
(
  struct fb_planeinfo_s *pinfo,
  const struct nxgl_rect_s *rect,
  struct nxgl_point_s *offset)
{
  ...
  leadmask = (uint8_t)(0xff >> (8 - ((rect->pt1.x) & 1)));
  tailmask = (uint8_t)(0xff << (8 - ((rect->pt2.x - 1) & 1)));
  sline = pinfo->fbmem + rect->pt1.y * stride + ((rect->pt1.x) >> 1);
  dline = pinfo->fbmem + offset->y * stride + ((offset->x) >> 1);

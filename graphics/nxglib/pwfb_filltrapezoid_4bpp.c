  ...
  uint8_t color)
{
  ...
  b16_t dx1dy;
  b16_t dx2dy;
  uint8_t mpixel = ((uint8_t)(color) << 4 | (color));
  uint8_t mask;
  int lnlen;
  stride = bwnd->stride;
  x1 = trap->top.x1;
      ...
      if (x1 <= x2 && ix2 >= bounds->pt1.x && ix1 <= bounds->pt2.x)
        {
          ...
          if (lnlen > 0)
            {
              { uint8_t *_ptr = ( uint8_t*)(dest); int _nby = ((lnlen) >> 1); while (_nby--) { *_ptr++ = ((uint8_t)color); } };
            }
        }
      line += stride;
      x1 += dx1dy;

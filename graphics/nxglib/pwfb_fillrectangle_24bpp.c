  ...
  uint32_t color)
{
  ...
  while (rows-- > 0)
    {
      { uint8_t *_ptr = ( uint8_t*)(line); nxgl_coord_t _npix = (width); while (_npix--) { *_ptr++ = ((uint32_t)color); *_ptr++ = ((uint32_t)color) >> 8; *_ptr++ = ((uint32_t)color) >> 16; } };
      line += stride;
    }
}

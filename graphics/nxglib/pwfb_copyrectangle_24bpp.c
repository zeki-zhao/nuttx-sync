  ...
  unsigned int srcstride)
{
  ...
  sline = ( const uint8_t *)src +
          (((dest->pt1.x - origin->x) << 1) + (dest->pt1.x - origin->x)) +
          (dest->pt1.y - origin->y) * srcstride;
  dline = ( uint8_t *)bwnd->fbmem + dest->pt1.y * deststride +
          (((dest->pt1.x) << 1) + (dest->pt1.x));
  while (rows--)
    {
      { uint8_t *_dptr = ( uint8_t*)((uint32_t *)dline); uint8_t *_sptr = ( uint8_t*)((uint32_t *)sline); nxgl_coord_t _npix = (width); while (_npix--) { *_dptr++ = *_sptr++; *_dptr++ = *_sptr++; *_dptr++ = *_sptr++; } };
      dline += deststride;
      sline += srcstride;
    }
}
